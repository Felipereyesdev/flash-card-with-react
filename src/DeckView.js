    ...
    <div>
      <nav>
        ...
        <div className="mb-4">
            ...
            <button className="btn btn-secondary pl-1 mr-1" onClick ={() => history.push(`/decks/${deck.id}/edit`)}>edit</button>
            <button className="btn btn-primary mr-1" onClick ={() => history.push(`/decks/${deck.id}/study`)}>study</button>
            <button className="btn btn-primary mr-1" onClick ={() => history.push(`/decks/${deck.id}/cards/new`)}>add card</button>
      ...
      {cards.map((card,index) => (
        <div key={card.id} className="card px-5 py-3 mb-3">
          ...
          <button className="btn btn-secondary pl-1 mr-1" onClick ={() => history.push(`/decks/${deckId}/cards/${card.id}/edit`)}>edit</button>
          <button className="btn btn-danger ml-auto" onClick ={() =>{
            console.log("This is firing")
            handleCardDelete(card.id)}}>delete</button>
            </div>
        </div>
      ))}
    </div>
